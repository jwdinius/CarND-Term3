    ...
    for i in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            _, loss = sess.run([train_op, cross_entropy_loss], 
                               feed_dict={input_image: image, correct_label: label, keep_prob: (1.-DROPOUT), learning_rate: ETA})
...
def run():
    num_classes = 2 # this could be changed to do some cool stuff later!
    image_shape = (160, 576)
    data_dir = './data'
    runs_dir = './runs'
